...
class Train(GoVariable):
    ...
    def train(self):
        ...
        with tf.device("/gpu:0"):
            ...
            with tf.variable_scope('large_policy'):
        ...
        with tf.device("/cpu:0"):
            ...
            def bias_variable(shape, variable_name=None):
                ...
                return tf.Variable(initial, name=variable_name)
            with tf.variable_scope('large_policy'):
                W_conv10 = weight_variable([3, 3, n_out, n_out])
                b_conv10 = bias_variable([n_out])
                h_conv10 = tf.nn.relu(conv2d(conv9_bn, W_conv10) + b_conv10)
                conv10_bn = batch_normalization(n_out,h_conv4)

                W_conv11 = weight_variable([3, 3, n_out, n_out])
                b_conv11 = bias_variable([n_out])
                h_conv11 = tf.nn.relu(conv2d(conv10_bn, W_conv11) + b_conv11)
                conv11_bn = batch_normalization(n_out,h_conv11)

                W_conv12 = weight_variable([3, 3, n_out, n_out])
                b_conv12 = bias_variable([n_out])
                h_conv12 = tf.nn.relu(conv2d(conv11_bn, W_conv12) + b_conv12)
                conv12_bn = batch_normalization(n_out,h_conv12)

                W_conv13 = weight_variable([3, 3, n_out, n_out])
                b_conv13 = bias_variable([n_out])
                h_conv13 = tf.nn.relu(conv2d(conv12_bn, W_conv13) + b_conv13)
                conv13_bn = batch_normalization(n_out,h_conv13)


                conv13_bn_flat = tf.reshape(conv13_bn, [-1, 23 * 23 * n_out])

                weight_fully_connected1 = weight_variable([23 * 23 * n_out, 1200])
                bias_fc1 = weight_variable([1200])
                hidden_fully_connect1 = tf.nn.relu(tf.matmul(conv13_bn_flat, weight_fully_connected1) + bias_fc1)

                keep_prob = tf.placeholder("float")
                h_fc1_drop = tf.nn.dropout(hidden_fully_connect1, keep_prob)

                weight_fully_connected2 = weight_variable([1200, 361])
        ...
        saver = tf.train.Saver(self.get_particular_variables('large_policy'))
                            ...
                            for game in collection:
                                for node in game:
                                    ...
                                    if (batch_count_sum_all + 98) % 20 == 0 and batch_count_num > 50:
                                        ...
                                        summary_str, accuracy_value, cross_entropy_value = sess.run([merged, accuracy, cross_entropy],feed_dict={x_input: xTrain, y_: yTrain, keep_prob: 0.5})
                                        train_writer.add_summary(summary_str, step)
                                        train_writer.flush()
                                    ...
                                    if train_count_num > 1500:
                                        ...
                                        ckpt_num += 1
                                        print "SAVED!"
                                        saver.save(sess, './Network_Backup/large_policy' + str(ckpt_num))
