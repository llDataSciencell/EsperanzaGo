    ...
    def train(self):
        ...
        with tf.device("/gpu:1"):
            ...
            with tf.variable_scope('large_policy'):
                ...
                conv9_bn_flat = tf.reshape(conv9_bn, [-1, 23 * 23 * n_out])

                weight_fully_connected1 = weight_variable([23 * 23 * n_out, 1200])
                bias_fc1 = weight_variable([1200])
                hidden_fully_connect1 = tf.nn.relu(tf.matmul(conv9_bn_flat, weight_fully_connected1) + bias_fc1)
