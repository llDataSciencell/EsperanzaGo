...
class ForwardPropNetwork(GoVariable):
    ...
    def train(self,sess):
        ...
        with tf.device("/cpu:0"):
            ...
            with tf.variable_scope('policy_auto'):
                ...
                self.h_conv2 = tf.nn.relu(self.conv2d(self.conv1_bn, self.W_conv2) + self.b_conv2)
                ...
                self.h_conv3 = tf.nn.relu(self.conv2d(self.conv2_bn, self.W_conv3) + self.b_conv3)
                ...
                self.h_conv4 = tf.nn.relu(self.conv2d(self.conv3_bn, self.W_conv4) + self.b_conv4)
                self.conv4_bn = self.batch_normalization(n_out,self.h_conv4)

                self.conv4_bn_flat = tf.reshape(self.conv4_bn,[-1,23*23*n_out])
                ...
                self.hidden_fully_connect1 = tf.nn.relu(tf.matmul(self.conv4_bn_flat, self.weight_fully_connected1) + self.bias_fc1)
        ...
        self.saver = tf.train.Saver(self.get_particular_variables('policy_auto'))
    ...
    def _main(self,sess):
        self.saver.restore(sess,'./Network_Backup/batchnorm_legal')
        #self.test(sess)
        #print "gtp_io"
