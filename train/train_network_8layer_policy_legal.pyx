...
class Train8LayerPolicyLegal(GoVariable):
    ...
    def train(self):
        ...
        with tf.device("/gpu:0"):
            with tf.variable_scope('large'):
                ...
                W_conv1 = self.weight_variable([5, 5, 8, n_out])  # [5,5,6,50]
                b_conv1 = self.bias_variable([n_out])  # [50]
                ...
                # 3x3フィルタで64チャネルを出力
                W_conv2 = self.weight_variable([3, 3, n_out, n_out])  # [3,3,50,100]
                b_conv2 = self.bias_variable([n_out])
                h_conv2 = tf.nn.relu(self.conv2d(h_conv1, W_conv2) + b_conv2)
                #h_pool2 = self.max_pool_2x2(h_conv2)
                # 次の層の畳み込み処理を行う前に、paddingを実行。

                ###5層目 畳み込み層
                W_conv3 = self.weight_variable([3, 3, n_out, n_out])
                b_conv3 = self.bias_variable([n_out])
                h_conv3 = tf.nn.relu(self.conv2d(h_conv2, W_conv3) + b_conv3)
                #h_pool3 = self.max_pool_2x2(h_conv3)

                ###7層目　畳み込み層
                W_conv4 = self.weight_variable([3, 3, n_out, n_out])
                b_conv4 = self.bias_variable([n_out])
                h_conv4 = tf.nn.relu(self.conv2d(h_conv3, W_conv4) + b_conv4)

                W_conv5 = self.weight_variable([3, 3, n_out, n_out])
                b_conv5 = self.bias_variable([n_out])
                h_conv5 = tf.nn.relu(self.conv2d(h_conv4, W_conv5) + b_conv5)

                W_conv6 = self.weight_variable([3, 3, n_out, n_out])
                b_conv6 = self.bias_variable([n_out])
                h_conv6 = tf.nn.relu(self.conv2d(h_conv5, W_conv6) + b_conv6)

                W_conv7 = self.weight_variable([3, 3, n_out, n_out])
                b_conv7 = self.bias_variable([n_out])
                h_conv7 = tf.nn.relu(self.conv2d(h_conv6, W_conv7) + b_conv7)

                W_conv8 = self.weight_variable([3, 3, n_out, n_out])
                b_conv8 = self.bias_variable([n_out])
                h_conv8 = tf.nn.relu(self.conv2d(h_conv7, W_conv8) + b_conv8)
                #h_pool4 = self.max_pool_2x2(h_conv4)
        with tf.device("/cpu:0"):
            with tf.variable_scope('large'):
                #h_pool4_flat = tf.reshape(h_pool4, [-1, 23 * 23 * n_out])  # 29*29*100
                h_pool8_flat = tf.reshape(h_conv8, [-1, 23 * 23 * n_out])
                weight_fully_connected1 = self.weight_variable([23 * 23 * n_out, 1000])
                bias_fc1 = self.weight_variable([1000])
                hidden_fully_connect1 = tf.nn.relu(tf.matmul(h_pool8_flat, weight_fully_connected1) + bias_fc1)
        ...
        saver = tf.train.Saver(self.get_particular_variables('large'))
        ...
        with tf.Session() as sess:
            ...
            saver.restore(sess, './Network_Backup/large_policy_legal_move')
            print "restore END"
                            ...
                            for game in collection:
                                for node in game:
                                    ...
                                    if train_count_num > 1500:
                                        ...
                                        ckpt_num += 1
                                        print "SAVED!"
                                        saver.save(sess, './Network_Backup/large_policy_legal_move' + str(ckpt_num))
