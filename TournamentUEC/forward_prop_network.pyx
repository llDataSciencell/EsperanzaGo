...
class ForwardPropNetwork(GoVariable):
    ...
    def train(self,sess):
        ...
        with tf.device("/cpu:0"):
            ...
            with tf.variable_scope('large'):
                ...
                self.x_image = tf.reshape(self.x_input, [-1, 19, 19, 8])
                ...
                self.W_conv1 = self.weight_variable([5, 5, 8, n_out])  #[5,5,6,50]
                self.b_conv1 = self.bias_variable([n_out])#[50]
                self.h_conv1 = tf.nn.relu(self.conv2d(self.x_image_pad, self.W_conv1) + self.b_conv1)
                #self.conv1_bn = self.batch_normalization(n_out,self.h_conv1)
                #次の層の畳み込み処理を行う前に、paddingを実行。
                # 3層目 畳み込み層

                # パッチサイズ縦、パッチサイズ横、入力チャネル（枚数）、出力チャネル（出力の枚数）
                # 3x3フィルタで64チャネルを出力
                self.W_conv2 = self.weight_variable([3, 3, n_out, n_out])  #[3,3,50,100]
                self.b_conv2 = self.bias_variable([n_out])
                self.h_conv2 = tf.nn.relu(self.conv2d(self.h_conv1, self.W_conv2) + self.b_conv2)
                #self.conv2_bn = self.batch_normalization(n_out,self.h_conv2)
                #次の層の畳み込み処理を行う前に、paddingを実行。

                ###5層目 畳み込み層
                self.W_conv3 = self.weight_variable([3, 3, n_out, n_out])
                self.b_conv3 = self.bias_variable([n_out])
                self.h_conv3 = tf.nn.relu(self.conv2d(self.h_conv2, self.W_conv3) + self.b_conv3)
                #self.conv3_bn = self.batch_normalization(n_out,self.h_conv3)

                ###7層目　畳み込み層
                self.W_conv4 = self.weight_variable([3, 3, n_out, n_out])
                self.b_conv4 = self.bias_variable([n_out])
                self.h_conv4 = tf.nn.relu(self.conv2d(self.h_conv3, self.W_conv4) + self.b_conv4)

                self.W_conv5 = self.weight_variable([3, 3, n_out, n_out])
                self.b_conv5 = self.bias_variable([n_out])
                self.h_conv5 = tf.nn.relu(self.conv2d(self.h_conv4, self.W_conv5) + self.b_conv5)

                self.W_conv6 = self.weight_variable([3, 3, n_out, n_out])
                self.b_conv6 = self.bias_variable([n_out])
                self.h_conv6 = tf.nn.relu(self.conv2d(self.h_conv5, self.W_conv6) + self.b_conv6)

                self.W_conv7 = self.weight_variable([3, 3, n_out, n_out])
                self.b_conv7 = self.bias_variable([n_out])
                self.h_conv7 = tf.nn.relu(self.conv2d(self.h_conv6, self.W_conv7) + self.b_conv7)

                self.W_conv8 = self.weight_variable([3, 3, n_out, n_out])
                self.b_conv8 = self.bias_variable([n_out])
                self.h_conv8 = tf.nn.relu(self.conv2d(self.h_conv7, self.W_conv8) + self.b_conv8)
                #self.conv4_bn = self.batch_normalization(n_out,self.h_conv4)
        with tf.device("/cpu:0"):
            with tf.variable_scope('large'):
                self.h_pool8_flat = tf.reshape(self.h_conv8,[-1,23*23*n_out])
                self.weight_fully_connected1=self.weight_variable([23*23*n_out,1000])
                self.bias_fc1=self.weight_variable([1000])
                #TODO caution hiden is often used.not hidden
                self.hidden_fully_connect1 = tf.nn.relu(tf.matmul(self.h_pool8_flat, self.weight_fully_connected1) + self.bias_fc1)

                # ドロップアウトを指定
                self.keep_prob = tf.placeholder("float")
                self.h_fc1_drop = tf.nn.dropout(self.hidden_fully_connect1, self.keep_prob)

                self.weight_fully_connected2 = self.weight_variable([1000,361])
                self.bias_fc2 = self.bias_variable([361])
                self.y_conv = tf.nn.softmax(tf.matmul(self.h_fc1_drop,self.weight_fully_connected2) + self.bias_fc2)
        ...
        self.saver = tf.train.Saver(self.get_particular_variables('large'))
    ...
    def search_deep_learning(self,sess, go_state_obj, current_player):
        ...
        output_array = sess.run(self.y_conv, feed_dict={self.x_input: xTrain, self.keep_prob: 1.0})
    ...
    def _main(self,sess):
        self.saver.restore(sess,'./Network_Backup/large_policy_legal_move')
        #self.test(sess)
        #print "gtp_io"
