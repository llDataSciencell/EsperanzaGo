...
class GoStateObject(GoVariable):
    def __init__(self, board=None, ko_place=None, turns_since_board=None, turns_num=None, agehama=None, last_move=None):
        if board is not None:
            ...
            self.turns_num = 0
            self.agehama = [0, 0]  # agehama[0]:Black, agehama[1]:white
            self.last_move = None
...
class Go(State, GoVariable):
    ...
    def calcTerritory(self, territoryBoard, go_state_obj):
        ret = None
        black_count = 0
        white_count = 0
        ...
        black_count += go_state_obj.agehama[0]
        white_count += go_state_obj.agehama[1]
    ...
    def make_final_captured_board(self ,go_state_obj):
        ...
        #sys.stderr.write(str(self.print_board(go_state_obj)))
        return go_state_obj
    ...
    def count_territory(self, go_state_obj):
        ...
        score_tuple = self.calcTerritory(territoryBoard, go_state_obj)   # Territoryの計算
        return score_tuple
    ...
    def copy_board_obj(self, go_state_obj):
        ...
        self.agehama = go_state_obj.agehama
        self.last_move = go_state_obj.last_move
    ...
    def move(self, go_state_obj, player, next_move):
        ...
        if next_move == self._PASS:  # passの時の処理
            return GoStateObject([[self._board[y][x] for x in xrange(self._SIZE)] for y in xrange(self._SIZE)], self._ko_place, self._turns_since_board,self.turns_num, self.agehama, last_move=next_move)
        else:
            flips_list.extend(self._flips(player, next_move))
            mycolor = int(player.player_id)
            self.agehama[mycolor] += len(flips_list)  # agehama update(2017/03/15)
            ...
            return GoStateObject([[self._board[y][x] if (x, y) not in flips_list else self._EMPTY for x in xrange(self._SIZE)] for y in xrange(self._SIZE)], self._ko_place, self._turns_since_board,self.turns_num, self.agehama, last_move=next_move)
    ...
    def move_and_return_state(self, go_state_obj, player, next_move):
        ...
        flips_list = []
        self._ko_place = []
        if next_move == self._PASS:
            return GoStateObject([[aBoard[y][x] for x in xrange(self._SIZE)] for y in xrange(self._SIZE)], self._ko_place, self._turns_since_board,self.turns_num, self.agehama, last_move=next_move)
        else:
            flips_list.extend(self._flips(player, next_move))
            mycolor = int(player.player_id)
            self.agehama[mycolor] += len(flips_list)  # agehama update(2017/03/15)
            # print("flips_list:" + str(flips_list))
            if self._check_ko(player, next_move, len(flips_list)):
                self._ko_place.append(flips_list[0])
            aBoard[next_move[1]][next_move[0]] = player.player_id
            return GoStateObject([[aBoard[y][x] if (x, y) not in flips_list else self._EMPTY for x in xrange(self._SIZE)] for y in xrange(self._SIZE)], self._ko_place,self._turns_since_board,self.turns_num, self.agehama, last_move=next_move)
...
def agehama_test():
    from main import read_move
    from search import MontecarloSearch
    import random

    players = [Player(0.0, 'random'), Player(1.0, 'random')]
    players[0].next_player = players[1]
    players[1].next_player = players[0]
    player = players[0]
    rule = Go()
    ...
    while True:
        if player.player_name == 'human':
            next_move = read_move(go_state_obj, rule, player)
            go_state_obj = rule.move(go_state_obj, player, next_move)
        elif player.player_name == 'AI':
            (go_state_obj, next_move) = search_algorithm.next_move(go_state_obj, player)
            print("%s's move for %s: %s" % (str(player).title(),
                                        str(player.player_name),
                                        next_move))
        else:
            # random
            next_moves = rule.next_moves(go_state_obj, player, True)
            next_move = random.choice(next_moves)
            go_state_obj = rule.move(go_state_obj, player, next_move)

        print(rule.print_board(go_state_obj))
        go_state_obj.print_state()
        player = player.next_player_func()

      # ko check
        if len(go_state_obj._ko_place) > 0:
            print 'kokokokokokokokokokokoko\n' * 10

       # pass check
        if go_state_obj.last_move == 'pass':
            passed += 1
            if passed > 1:
                print 'game end'
                break
